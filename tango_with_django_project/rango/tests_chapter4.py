...
class Chapter4StaticMediaTests(TestCase):
  ...
  Also tests for the two required files -- rango.jpg and cat.jpeg.
  """
    ...
    Also checks for the presence of cat.jpeg.
    """
        does_media_dir_exist = os.path.isdir(self.media_dir)
        does_cat_jpg_exist = os.path.isfile(os.path.join(self.media_dir, 'cat.jpeg'))
        ...
        self.assertTrue(does_cat_jpg_exist,
                        f"{FAILURE_HEADER}We couldn't find the cat.jpeg image in /media/. Check the file extension; this is a common pitfall. It should .jpg. Not .png, .gif, or .jpeg!{FAILURE_FOOTER}")
...
class Chapter4ExerciseTests(TestCase):
  ...
  """
    ...
    """
        required_pattern = f"<img src=\"{settings.MEDIA_URL}cat.jpeg\" alt=\"Picture of a Cat.?\" />"
